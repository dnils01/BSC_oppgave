...
void HD44780_PCF8574_E_pulse (char data)
{
  // E pulse
  // ----------------------------------
  I2C_SendByte(data | PCF8574_PIN_E);
...
char HD44780_PCF8574_PositionXY (char addr, char x, char y)
{
  ...
  if (y == 0) {
    ...
    // send instruction 2nd row
    HD44780_PCF8574_SendInstruction(addr, (HD44780_POSITION | (HD44780_ROW3_START + x)));
    
	 } else if (y == 3) {
	 // send instruction 2nd row
	 HD44780_PCF8574_SendInstruction(addr, (HD44780_POSITION | (HD44780_ROW4_START + x)));
 }
  // success
  return PCF8574_SUCCESS;
}

char HD44780_PCF8574_ClearLine(char addr, char y){
	 HD44780_PCF8574_PositionXY (addr, 0, y);	
